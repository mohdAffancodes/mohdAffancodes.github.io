...
function fnSlide() {
    let borderWidth = parseInt(
        getComputedStyle(parent).borderRightWidth.slice(0, -2)
    );
    //console.log(borderWidth);
    if (sliders[current].offsetWidth <= 0 && gameOver == false) {
        fnGameOver();
    }

    for (let i = current; i < divNo; i++) {
        if (
            sliders[i].offsetLeft + sliders[i].offsetWidth >=
            parent.offsetWidth - borderWidth * 2
        ) {
            speed = -2;
...
function fnCutOff() {
    if (current >= 1) {
        divBelow = current - 1;
        if (
            sliders[current].offsetLeft + sliders[current].offsetWidth <
                sliders[divBelow].offsetLeft ||
            sliders[current].offsetLeft >
                sliders[divBelow].offsetLeft + sliders[current].offsetWidth
        ) {
            fnGameOver();
        }
        ...
        if (gameOver == false) {
            if (
                sliders[current].offsetLeft + sliders[current].offsetWidth <
                sliders[divBelow].offsetLeft + sliders[divBelow].offsetWidth
            ) {
                //left side
                let differenceOfLefts =
                    sliders[divBelow].offsetLeft - sliders[current].offsetLeft; //difference

                sliders[current].style.left = sliders[divBelow].offsetLeft;

                for (let i = current; i < divNo; i++) {
                    sliders[i].style.width =
                        sliders[i].offsetWidth - differenceOfLefts;
                }
            } else if (
                sliders[current].offsetLeft > sliders[divBelow].offsetLeft
            ) {
                //right side
                let differenceOfRights =
                    sliders[current].offsetLeft +
                    sliders[divBelow].offsetWidth -
                    (sliders[divBelow].offsetLeft +
                        sliders[divBelow].offsetWidth); //difference

                sliders[current].style.left = sliders[divBelow].offsetLeft;

                for (let i = current; i < divNo; i++) {
                    sliders[i].style.width =
                        sliders[i].offsetWidth - differenceOfRights;
                }
                sliders[current].style.left =
                    sliders[divBelow].offsetLeft +
                    sliders[divBelow].offsetWidth -
                    sliders[current].offsetWidth;
            }
        }
    }
}
...
function fnGameOver() {
    ...
    if (
        ...
        zoneDivs[1].innerHTML =
            "<p style='text-align:center;'>" + "TOUCH TO RESTART" + "</p>";
    } else {
        zoneDivs[1].innerHTML =
