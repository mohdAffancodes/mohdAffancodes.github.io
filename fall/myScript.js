...
function blocks() {
    ...
    for (
        let i = 0, b = 1;
        i < blockArray.length, b < blockArray.length;
        i++, b++
    ) {
        blockArray[b].style.backgroundColor = "white";
        blockArray[b].style.top =
            blockArray[i].offsetTop + blockArray[i].offsetHeight + 100 + "px";
    }
    //Gap divs
    for (var i = 0; i < blockArray.length; i++) {
        gapArray[i].style.top = blockArray[i].style.top;
        gapArray[i].style.width = randomNumber(60, 100) + "px";
        gapArray[i].style.left =
            randomNumber(0, p.offsetWidth - gapArray[i].offsetWidth) + "px";
    }
}
...
function game() {
    ...
    for (let i = 0; i < blockArray.length; i++) {
        ...
        if (blockArray[i].offsetTop <= 0 && gapArray[i].offsetTop <= 0) {
            ...
            blockArray[i].style.top =
                p.offsetHeight - 6 - blockArray[i].offsetHeight + "px";
            gapArray[i].style.top =
                p.offsetHeight - 6 - gapArray[i].offsetHeight + "px";
            gapArray[i].style.width = randomNumber(60, 100) + "px";
            gapArray[i].style.left =
                randomNumber(0, p.offsetWidth - gapArray[i].offsetWidth) + "px";
            downArray[i] = false;
            ...
            blockArray[i].style.top =
                blockArray[i].offsetTop - blockSpeed + "px";
            gapArray[i].style.top = gapArray[i].offsetTop - blockSpeed + "px";
        }
        //All about ball w.r.t block & gap
        if (
            ball.offsetTop + ball.offsetHeight >= blockArray[i].offsetTop &&
            downArray[i] == false
        ) {
            if (
                ball.offsetLeft >= gapArray[i].offsetLeft &&
            ...
            if (
                ball.offsetLeft <= gapArray[i].offsetLeft &&
                ball.offsetTop + ball.offsetHeight > gapArray[i].offsetTop &&
                ball.offsetTop <
                    gapArray[i].offsetTop + gapArray[i].offsetHeight &&
                downArray[i] == true
            ) {
                ...
                ball.style.left =
                    gapArray[i].offsetLeft +
                    gapArray[i].offsetWidth -
                    ball.offsetWidth +
                    "px";
            }
        }
        ...
        for (let i = 0; i < blockArray.length; i++) {
            if (key == 37 || key == 97 || key == 65) {
                ...
                if (ball.offsetLeft >= p.offsetWidth - 6 - ball.offsetWidth) {
                    ball.style.left =
                        p.offsetWidth - 6 - ball.offsetWidth + "px";
                    collide.play();
                    stuckInRight == true;
                } else if (
                    ball.offsetLeft + ball.offsetWidth >=
