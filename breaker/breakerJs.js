...
for (let i = 0; i < arrayArray.length; i++) {
   arrayArray[i] = wallArray.map((value) => {
      return value;
   });
}
...
function animate() {
   ...
   if (
      ...
      ballX < paddleX + ballRadius + PADDLE_THICKNESS &&
      ballY > paddleY - ballRadius &&
      ...
      if (ballY - ballRadius > PADDLE_HEIGHT + (PADDLE_HEIGHT - paddleY)) {
         ...
         if (
            ...
            ballY < paddleY + PADDLE_HEIGHT &&
            ballX > paddleX &&
            ballX < paddleX + PADDLE_THICKNESS
         ) {
            ballY -= ballRadius;
         }
...
function drawEverything() {
   ...
   if (previewScreen == true && fallNo < reset) {
      ...
      writeTxt(
         "white",
         "20px Courier",
         "To PLAY keep the mouse in the GAME area",
         canvas.width / 2 - 230,
         canvas.height / 2 + 50
      );
...
function wallCollision(x, y, arr) {
   for (x = 0; x < arr.length; x++) {
      if (
         ballX - ballRadius < arr[x] + WALL_THICKNESS &&
         ballX + ballRadius > arr[x] &&
         ballY - ballRadius < y + WALL_HEIGHT &&
         ballY + ballRadius > y
      ) {
         ballRebound(arr, x);
         audio3.play();
...
function ballRebound(arr, x) {
   ...
   if (ballX < 15) {
      ...
      ballSpeedX = Math.round(angleX * 0.4);
   }
   ballSpeedY = -ballSpeedY;
}
