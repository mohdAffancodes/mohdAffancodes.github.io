...
function start() {
	...
	canvas.addEventListener('mousedown', e => {
		if(displayWinScreen == true) {location.reload()}
	})
...
function randomNumber(min, max) { // Function to generate random number 
...
function paddleAI() {
	if(ballY-40 > rightPaddleY+(PADDLE_HEIGHT/2)) {rightPaddleY += 12.5 ;}
		//+= is the same as rightPaddleY = rightPaddleY + 5 i.e. A=A+B == A += B	
	else if(ballY+40 < rightPaddleY+(PADDLE_HEIGHT/2)) {rightPaddleY -= 12.5 ;}
		//-= is the same as rightPaddleY = rightPaddleY - 5 i.e. A=A-B == A -= B
}

function moveEverything() {
	raf(moveEverything);
	if (displayWinScreen == true) {return}
	...
	if(ballX < (PADDLE_THICKNESS + 20)) {
		if (ballX < 0) {
			...
			if((leftPaddleX && ballX < leftPaddleX + PADDLE_HEIGHT) && (ballX > leftPaddleY && ballX < leftPaddleY + PADDLE_THICKNESS)) {
	...
	if(ballX > canvas.width-(PADDLE_THICKNESS + 20)) {
		if(ballX > rightPaddleX && ballX < rightPaddleX + PADDLE_THICKNESS && ballY > (rightPaddleY + 10) && ballY < (rightPaddleY + PADDLE_HEIGHT) - 10) {
			console.log("bichme");	
			ballX = canvas.width/2;
		...
		if (ballX > canvas.width) {
			...
			if((rightPaddleX && ballX < rightPaddleX + PADDLE_HEIGHT) && (ballX > rightPaddleY && ballX < rightPaddleY + PADDLE_THICKNESS)) {
				ballSpeedY = +ballSpeedY;
			}
		}
	}
	if(ballY < 10) {ballSpeedY = -ballSpeedY;}
	if(ballY > canvas.height-10) {ballSpeedY = -ballSpeedY;}
}
...
function drawNet() {
	...
	for (let i = 10; i < canvas.height; i+=40) {
		drawRect(canvas.width/2-(NET_THICKNESS/2),i,NET_THICKNESS,NET_HEIGHT,'white');
	}
}
...
function drawEverything() {
	...
	if (displayWinScreen == true) {
		if (p1Score >= WIN_SCORE) {
			...
			//player name
			if(pN == "") {
				writeTxt('white','50px "Press Start 2P",cursive',"Player 1 WON",100,canvas.height/2);	
			}
			else {
				writeTxt('white','50px "Press Start 2P",cursive',pN + " WON",100,canvas.height/2);	
			}
			writeTxt('white','15px Courier',"Click to Continue",(canvas.width/2)-70,canvas.height - 50);
		}
	...
	drawRect(rightPaddleX,rightPaddleY,PADDLE_THICKNESS,PADDLE_HEIGHT,'white');
	...
	writeTxt('white','25px Courier',"Computer's Score = " + p2Score,canvas.width-330,50)
}
